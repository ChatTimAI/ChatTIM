...
class GenericApiChatModel(ChatModel):
    ...
    def generate(
        ...
        usage = GenericApiChatModel.get_usage(res.usage)
    ...
    def generate_stream(
        ...
        for chunk in stream:
            usage = GenericApiChatModel.get_usage(chunk.usage)
...
class AsyncGenericApiChatModel(AsyncChatModel):
    ...
    async def generate(
        ...
        message_content = res.choices[0].message.content or ""
        usage = AsyncGenericApiChatModel.get_usage(res.usage)
        return ModelResponse(content=message_content, usage=usage)
    ...
    async def generate_stream(
        ...
        async for chunk in stream:
            usage = AsyncGenericApiChatModel.get_usage(chunk.usage)
            if len(chunk.choices) == 0:
                # No more messages
                yield ModelResponseChunk(usage=usage, done=True)
                continue
